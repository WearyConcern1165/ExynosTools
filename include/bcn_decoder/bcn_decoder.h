...
/// Describes a command-buffer decode from a compressed source buffer into an
/// existing destination image.
struct BufferImageDecodeInfo {
    ...
    const VkBufferImageCopy*   pRegions         = nullptr;
};
...
public:
    ...
    /// Records compute decode work into an existing command buffer, writing the
    /// decoded texels directly into an existing image.
    BcnResult recordDecodeBufferToImage(const BufferImageDecodeInfo& info);

    /// Releases cached image views created for inline decode on a given image.
    void releaseImageViews(VkImage image);
