...
void exynos_tfb_draw_indirect_byte_count(
    ...
    e->last_vertex_count = vertex_count;
    if (vertex_count == 0) {
...
uint32_t exynos_tfb_get_last_vertex_count(ExynosTfbEmulator* e) {
    if (!e || !e->enabled) {
        return 0;
    }

    pthread_mutex_lock(&e->lock);
    uint32_t vertex_count = e->last_vertex_count;
    pthread_mutex_unlock(&e->lock);
    return vertex_count;
}
