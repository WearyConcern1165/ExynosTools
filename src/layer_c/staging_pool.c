...
VkResult exynos_pool_init(ExynosStagingPool* pool,
    ...
    // V3.1: Increased defaults for smoother texture streaming
    pool->buffer_count = (config && config->buffer_count) ? config->buffer_count : 16u;
    pool->default_buffer_size = (config && config->default_buffer_size) ? config->default_buffer_size : (64u * 1024u * 1024u);
...
VkResult exynos_pool_acquire(ExynosStagingPool* pool,
    ...
    *out_buffer = NULL;
    ...
    for (;;) {
        ...
        // V3.1 FIX: Race condition fix.
        // The old code unlocked the mutex during vkWaitForFences, allowing
        // another thread to steal the buffer we just waited on.
        // Now we keep the mutex held during the short fence wait.
        ExynosStagingBuffer* oldest = NULL;
        for (uint32_t i = 0; i < pool->buffer_count; i++) {
        ...
        if (!oldest) {
            // No in_use buffer found (shouldn't happen), retry
            pthread_mutex_unlock(&pool->mutex);
            return VK_ERROR_INITIALIZATION_FAILED;
        }
        // Wait WITH mutex held — fence wait is max 100ms, acceptable
        VkResult wait_res = vkWaitForFences(pool->device, 1, &oldest->fence,
                                            VK_TRUE, 100000000ULL);
