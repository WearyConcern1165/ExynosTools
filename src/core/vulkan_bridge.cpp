...
// ═══════════════════════════════════════════════════════════════════════════
// vkGetDeviceProcAddr — V3.1 FIX: Proper device-level dispatch
// CRITICAL: This function receives VkDevice, NOT VkInstance.
// The old code redirected to wrap_vkGetInstanceProcAddr which expected 
// VkInstance, causing segfaults when DXVK passed a VkDevice handle.
// ═══════════════════════════════════════════════════════════════════════════

static VKAPI_ATTR PFN_vkVoidFunction VKAPI_CALL
wrap_vkGetDeviceProcAddr(VkDevice device, const char* pName)
{
    ensureInit();
    if (!pName) return nullptr;

    // Forward to real driver's device proc addr
...
wrap_vkGetInstanceProcAddr(VkInstance instance, const char* pName)
{
    ...
    if (strcmp(pName, "vkGetDeviceProcAddr") == 0)
        return (PFN_vkVoidFunction)wrap_vkGetDeviceProcAddr; // V3.1 FIX: correct signature
