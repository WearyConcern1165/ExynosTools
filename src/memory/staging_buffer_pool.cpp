...
BcnResult StagingBufferPool::acquire(VkDeviceSize size, StagingSlot** outSlot) {
    ...
    VkResult waitRes = vkWaitForFences(device_, 1, &waitSlot.fence, VK_TRUE, 100000000ULL); // 100ms
    if (waitRes == VK_TIMEOUT) {
        POOL_LOGW("Ring saturated after 100ms fence wait, allocating overflow staging buffer");
        StagingSlot* temp = new StagingSlot();
        BcnResult res = createSlot(size, true, temp);
        if (res != BcnResult::Success) {
            delete temp;
